...
- [x] `/api/runtime/status` now carries an explicit `effective_runtime` route summary for `chat_agent`, including GPU VLM versus Mac VLM route labels, wrapper/backend/queue endpoints, effective model, active profile, and non-secret VLM configuration truth; the cockpit uses that summary so GPU VLM mode does not collapse back to stale OpenRouter/default-provider wording
