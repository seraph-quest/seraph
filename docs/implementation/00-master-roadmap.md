...
- [x] Seraph now also exposes a first additive memory-provider surface, so extension-backed providers can be configured and toggled through the normal lifecycle, augment retrieval on a separate external-memory lane, and fall back cleanly without replacing canonical guardian memory.
...
- [x] active workflow triage now also exposes direct `use output`, `use failure`, `open best`, `continue best`, `compare best`, `draft next step`, and keyboard-first latest-branch/failure follow-through so routine workflow recovery does not require an inspector hop for every next action
