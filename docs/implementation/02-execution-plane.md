...
- [x] explicit managed-process stops and session teardown now terminate retained process groups within a bounded TERM/wait/KILL path, expose stopped plus remaining-descendant status, and remove the process handle and disposable runtime artifacts after cleanup
