...
- [ ] **Partial, branch-local #744; intended until the epic integration PR lands:** governed model-fabric streaming and the preflighted VLM adapter pass through `model_fabric.gpu_admission`, which admits typed priority classes with bounded FIFO queues, deadline/cancel handling, and one active serial GPU lease. `/api/runtime/status` exposes an operator-safe broker receipt; active cancellation requests cooperative callback cancellation and closes async iterators where supported, so a provider that ignores cancellation remains a residual risk. The durable job repository remains canonical, and this process-local seam does not claim durable queue persistence.
