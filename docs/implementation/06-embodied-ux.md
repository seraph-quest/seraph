...
- review findings fixed before the batch stayed complete:
  ...
  - fixed by switching triage and evidence labels plus action `aria-label`s to contextual names, tightening the inspector test scope, excluding ancestor/root fallbacks from latest-branch resolution, sorting evidence shortcuts by actual recency before binding `Shift+E`, and pinning the dense-control flows with self-contained cockpit coverage
- validation:
