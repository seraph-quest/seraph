...
_CANONICAL_ADAPTERS = (
    _review(
        ...
        "client.post",
        "canonical_adapter",
        "Leaf OpenAI-compatible transport invoked only after model-fabric preflight and synchronous GPU admission.",
        "Retain as a leaf adapter; new inference adapters must be called through model_fabric.execution or the shared GPU broker.",
    ),
    _review(
        "llm_runtime.py",
        4315,
        ...
        "client.stream",
        "canonical_adapter",
        "Leaf streaming transport owned by the governed model-fabric execution path; redirects remain disabled.",
    ...
    _review(
        ...
        364,
        "_analyze_with_local_vlm._transport",
        "client.post",
        "canonical_adapter",
        "Leaf VLM analyze-file transport called by the preflighted adapter and shared GPU admission wrapper.",
        "Retain as the VLM leaf transport; migrate only by replacing the governed adapter seam, never by adding a caller-side HTTP path.",
    ),
)
...
_REVIEWED_EXCEPTIONS = (
    _review(
        ...
        "Migrate remaining no-context and unregistered agent callers to an authenticated request context and shared GPU admission, then remove this exception.",
    ),
    _review(
        "llm_runtime.py",
        3520,
        ...
        "Migrate the fallback chain to canonical model-fabric targets with explicit identity and broker admission, then remove this exception.",
    ),
    _review(
        "llm_runtime.py",
        3936,
        ...
        "Migrate every remaining legacy completion caller to an authenticated canonical context and delete the direct LiteLLM branch.",
    ),
    _review(
        "llm_runtime.py",
        4014,
        ...
        "litellm.completion",
        "transitional_provider_call",
        "Legacy fallback completion transport is retained for compatibility when canonical context is absent.",
    ...
    _review(
        ...
        "Move probe transport into a capability-probe adapter that explicitly acquires the one-GPU broker lease; preserve no-fallback and probe receipts.",
    ),
    _review(
        "api/model_fabric_settings.py",
        501,
    ...
    _review(
        ...
        "Move probe transport behind the one-GPU capability-probe admission path and retain exact proof persistence.",
    ),
    _review(
    ...
    _review(
        "observer/screenshot_semantic_analysis.py",
        123,
        "_screenshot_semantic_analysis_health",
        "client.get",
        "health_or_metadata_probe",
        "Read-only screenshot VLM wrapper health probe used to report observer readiness.",
        "Keep outside inference admission while read-only; image analysis remains in the governed VLM adapter.",
    ),
    _review(
        "observer/screenshot_semantic_analysis.py",
        139,
        "_screenshot_semantic_analysis_queue_status",
        "client.get",
        "health_or_metadata_probe",
        "Read-only screenshot VLM queue-status probe used for observer scheduling metadata.",
        "Keep outside inference admission while read-only; queue state must not become an alternate dispatch path.",
    ),
    _review(
        "observer/screenshot_semantic_analysis.py",
        155,
        "_screenshot_semantic_analysis_backend_ready",
        "client.get",
        "health_or_metadata_probe",
        "Read-only screenshot VLM backend-health probe used to detect degraded readiness.",
        "Keep outside inference admission while read-only; image analysis must continue through the governed VLM adapter.",
    ),
    _review(
