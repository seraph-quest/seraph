...
- [x] explicit managed-process stops and session teardown now terminate retained process groups within a bounded TERM/wait/KILL path, expose stopped plus remaining-descendant status, and remove the process handle and disposable runtime artifacts after verified cleanup; same-group ownership is rechecked against the captured leader identity before each signal, while identity loss, stale PIDs, signaling failures, and surviving descendants remain operator-visible as unknown or failed with a recoverable handle
- [x] session deletion holds a process-cleanup fence across memory flush and the database transaction, rejects same-session background starts during teardown, and records requested/stopped/unknown/failed/conflict cleanup counts; same-group cleanup remains bounded to the verified `setsid()` group, so descendants that call `setsid()` remain a documented residual
