...
| Native software-engineering fixture runner | `backend/src/workflows/native_software_engineering.py`, `backend/tests/test_native_swe_operator_journey.py`, `backend/tests/fixtures/native_swe_repo/` | Real bounded path: exact service execution/session binding plus authenticated interactive approval ownership, source/patch/path/command/resource checks, durable job fences/artifacts, actual child-process cleanup, failure/readback evidence, and replay dedupe. Arbitrary repository execution, host isolation, and external provider transport remain excluded. |
