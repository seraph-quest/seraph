...
- [x] Seraph now also exposes a benchmark-proof surface: deterministic benchmark suites for memory/workflow continuity, browser or desktop execution, planning or retrieval reporting, and governed improvement are grouped into one operator-readable report, and governed self-evolution receipts now carry explicit benchmark-gate posture instead of relying on implicit eval-score interpretation.
