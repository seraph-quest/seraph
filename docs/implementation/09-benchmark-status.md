...
| Dense interface efficiency | `[ ]` | guardian workspace with linked state, workflow runs, interventions, audit, trace, fixed composer, persisted layouts, keyboard layout switching, Windows control surface, searchable capability palette, denser operator terminal, active triage, evidence shortcuts, workflow step-focus rows, direct step/output handoff actions, first keyboard-first inspect/approve/continue/redirect/workflow/output control, imported reach/governance surfaces, and a separate grouped Activity Ledger | stronger visual workflow-operating density, richer artifact/workflow history, and broader keyboard-first control beyond the current step-focus and evidence shortcuts | 06, 07 |
