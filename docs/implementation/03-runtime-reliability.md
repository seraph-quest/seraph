...
- [ ] **Partial, branch-local #775/#744; intended until the epic integration PR lands:** governed model-fabric streaming and synchronous calls use one bounded `remote_inference` admission contract with typed priorities, a queue bound of 64, one active remote request, a per-owner outstanding bound of 16, deadline/cancel handling, and owner/fencing-validated reconciliation shared across async loops and worker threads. Authenticated route metadata may reserve an owner spend estimate before dispatch; configured budgets reject before the provider callback, while unknown or uncertain remote results remain visible and retain possible cost liability until fenced reconciliation (including a single actual-cost settlement). `/api/runtime/status` exposes an operator-safe admission receipt; this process-local seam does not claim durable queue persistence until #743/#744 complete. GPU/VLM readiness is not an active requirement in this target. Validation is local-only for ordinary PRs; hosted Actions are not an authoritative gate under `AGENTS.md`.
- [ ] **Branch-local #743 implementation slice:** the existing `WorkflowRunState` row is now also the canonical typed durable job for `strategist_tick`: interval occurrences are admitted idempotently, queued, claimed with a fenced lease, checkpointed before inference, and terminally recorded with redacted decision/delivery effect receipts. Duplicate terminal occurrences do not re-run, and guardian-state/model failures remain visible as failed durable jobs. This is a bounded reference integration; it does not claim durable execution for every scheduler job or exactly-once external effects until the #743 acceptance evidence is reviewed and the epic integration branch lands.
