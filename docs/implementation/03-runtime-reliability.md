...
The Docker VLM wrapper forwards authenticated OpenAI-compatible `stream: true` chat requests as `text/event-stream` responses while keeping the request inside the same priority queue worker until the stream completes. Wrapper repo `seraph-quest/vlm-screenshot-server` commit `48eb4e3` implemented that SSE forwarding. The current local topology runs the Docker wrapper on the GPU host at `http://192.168.1.26:8001` and forwards to the GPU model server at `http://192.168.1.26:8000/v1`; current runtime validation must use that direct HTTP route as described in `docs/implementation/18-screenshot-folder-source.md`. Seraph still keeps a defensive non-stream fallback if the wrapper or backend rejects streaming, but that fallback is no longer the expected healthy path.
