...
- [x] session deletion holds a process-cleanup fence across memory flush and the database transaction, rejects same-session background starts and concurrent cleanup attempts during teardown, and records requested/stopped/unknown/failed/conflict cleanup counts; descendants created after the pre-stop snapshot or outside the process tree remain a documented residual
