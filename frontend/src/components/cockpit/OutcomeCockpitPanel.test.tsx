...
describe("OutcomeCockpitPanel", () => {
  it("binds endpoint-shaped goal, workflow, approval, route, evidence, and outcome fixtures", () => {
    ...
    renderFixture({}, { onApprove, onDeny, onInspectWork, onOpenPriorities: vi.fn() });
    ...
    expect(within(screen.getByTestId("outcome-approval-card")).getByText("2099-01-01T00:00:00Z")).toBeInTheDocument();
    expect(within(screen.getByTestId("outcome-approval-card")).getByText("goal 4")).toBeInTheDocument();
    expect(within(screen.getByTestId("outcome-result-card")).getByText("candidate-1")).toBeInTheDocument();
    expect(within(screen.getByTestId("outcome-result-card")).getByText("outcome-1")).toBeInTheDocument();
