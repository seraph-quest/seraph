...
- [x] Seraph now also exposes a governed additive memory-provider surface, so extension-backed providers can be configured and toggled through the normal lifecycle, augment retrieval and user/project modeling on a separate external-memory lane, suppress stale provider evidence before it reaches guardian context, surface capability-state governance explicitly, mirror canonical memories into advisory provider writeback after canonical persistence succeeds, and fall back cleanly without replacing canonical guardian memory.
...
- [x] Guardian state now carries memory signals, continuity threads, collaborators, recurring obligations, project timelines, corroboration sources, focus provenance, judgment risks, and learned timing, suppression, blocked-state, plus thread guidance, with live guardian learning now resolved across global, thread, project, and thread-plus-project scopes before policy-time arbitration instead of only first-pass focus and delivery bias.
- [x] Guardian learning now also treats stale supporting recall and stale execution pressure as first-class contradictions against the live project anchor, surfaces that pressure as blockers/open loops, and lowers receptivity further when negative intervention trends line up with those conflicts.
