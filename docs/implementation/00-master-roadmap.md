...
| 06. Embodied Interface | `[ ]` | The guardian cockpit is now the active browser shell, with a pane workspace, drag/resize plus grid snap, saved layout composition, session continuity restore, linked evidence, a searchable capability surface, a separate activity ledger window, richer live operator views, active triage, evidence shortcuts, keyboard-first command control, preflight/repair flows, and an extension studio shipped; deeper visual workflow debugging and denser step-level operator control are still left |
