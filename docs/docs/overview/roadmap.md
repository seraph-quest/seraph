...
| 03. Runtime Reliability | `[ ]` | Ordered fallbacks, runtime-path model overrides, local routing, runtime audit visibility, and deterministic eval foundations are shipped; richer provider selection, broader local routing, remaining edge coverage, and broader evals are still left |
...
- [x] ordered LLM fallbacks, health-aware rerouting, runtime-path model overrides, local helper and agent runtime paths, and broad runtime audit coverage
