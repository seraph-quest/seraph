...
- [ ] The biggest implementation gaps now cluster around deeper privileged execution isolation beyond the new field-scoped secret-ref and mutation-allowlist layer, denser step-level operator/debug ergonomics beyond the shipped multi-session workflow orchestration, deeper intervention learning beyond the new live-versus-procedural conflict diagnostics, richer world-model quality beyond the new ranking/arbitration plus ambiguity guardrail layer, production-grade hardening for the broadened reach surfaces, and cleaner provider-neutral capability/adapter seams.
