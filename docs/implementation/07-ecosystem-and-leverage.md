...
- [x] extension-backed memory providers now also expose operator-readable freshness/writeback quality controls, a provider-neutral adapter model with per-capability sync contracts, a canonical-memory/provenance contract, usefulness-ranked retrieval diagnostics, and guarded additive writeback semantics instead of acting like opaque “connected or not” external memory seams
