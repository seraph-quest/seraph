...
def raise_on_human_feedback(
    ...
    if human_issue_comments or human_review_comments or codex_review_comments:
        ...
        raise SystemExit(2)
    blocking_reviews = filter_blocking_reviews(reviews, review_requested_at)
    if blocking_reviews:
        print("Review states or requested changes detected. Address before merge.")
        raise SystemExit(2)
...
async def wait_for_codex(pr_number: int, checks_done: asyncio.Event) -> None:
    ...
    while True:
        issue_comments, review_comments, reviews, review_request_at = (
    ...
    while True:
        ...
        if not check_runs:
            ...
            if empty_seconds >= CHECKS_APPEAR_TIMEOUT_SECONDS:
                print("No checks detected after 120s; inspect CI configuration.")
                raise SystemExit(3)
            await asyncio.sleep(POLL_SECONDS)
            continue
        empty_seconds = 0
...
async def watch_pr() -> None:
    ...
    if is_merge_conflicting(pr):
        print(
            "PR has merge conflicts. Resolve against develop and push before "
    ...
    async def head_monitor() -> None:
        while True:
            ...
            if is_merge_conflicting(current):
                print(
                    "PR has merge conflicts. Resolve against develop and push "
                    "before running land_watch again.",
                )
                raise SystemExit(5)
            if current.head_sha != head_sha:
                print("PR head updated; pull, validate, and push again if needed.")
                raise SystemExit(4)
            await asyncio.sleep(POLL_SECONDS)
