...
| 9 | Guardian world-model learning quality v2 | M8 | parented under #422 | multi-signal learning, stale/conflicting evidence arbitration, salience/confidence calibration, and false-positive/false-negative accounting are represented in deterministic operator-visible fixture receipts |
| 10 | Governed extension marketplace hardening foundations | M9 | parented under #422 | pack review/verification receipts, compatibility and downgrade transition semantics, supply-chain/source-policy verdicts, provider trust downgrade blocking, and scaffold/doctor authoring ergonomics improve production-oriented foundations without claiming production marketplace security |
...
- shipped in the guardian learning-quality v2 slice on this branch: `guardian_world_model_learning_quality_v2` and `/api/operator/guardian-learning-quality` gate deterministic fixture receipts for multi-signal learning arbitration, stale/conflicting evidence suppression, salience/confidence calibration, false-positive and false-negative labels, operator replay visibility, and an explicit boundary that this is not live human-outcome validation or a completed adaptive world-model learning engine
- missing strategic gaps: live long-horizon human outcome studies, broader reach-surface intervention replay, and stronger adaptive learning beyond deterministic learning-quality and M8 receipts
...
- shipped in the M9 governed-ecosystem proof slice on this branch: `m9_governed_ecosystem` gates deterministic manifest governance, lifecycle review gates, connector degradation truth, marketplace governance flow, diagnostics/update triage, package review receipts, compatibility/downgrade transition semantics, supply-chain/source-policy verdicts, provider trust downgrade blocking, scaffold/doctor authoring ergonomics, and `/api/operator/m9-governed-ecosystem-benchmark` operator receipts with an explicit claim boundary: local deterministic governance proof, not competitor superiority or production marketplace security
- missing strategic gaps: broader managed-connector governance, production marketplace security, live third-party package review operations, external publisher/key infrastructure, and a stable taxonomy that keeps core features separate from extension contributions
- proof requirements: manifest and lifecycle validation, compatibility and enable/disable receipts, connector and extension health proofs, marketplace/review receipts, update transition blockers, and operator surfaces that show why a capability exists, where it came from, what trust level it carries, which supply-chain verdict applies, and which claim boundary applies
