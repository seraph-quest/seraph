...
- [x] Seraph now also has a first workspace-level long-running workflow orchestration surface: the operator API and cockpit group active workflow supervision by thread, surface lead-run step focus and recoverability across multiple sessions, keep inspect/continue/failure-context/open-thread or next-step control available outside the currently inspected run, and compact long-running state into explicit capsules with visible-versus-compacted step counts plus preserved retry/checkpoint/repair paths.
