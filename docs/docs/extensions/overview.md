...
- the first shipped health contract covers packaged MCP connectors, managed connectors, observer definitions, and channel adapters; managed connectors now distinguish `requires_config`, `disabled`, and `ready`, while deeper reach-surface runtime behavior continues to land in later slices
