...
| Native software-engineering fixture runner | `backend/src/workflows/native_software_engineering.py`, `backend/tests/test_native_swe_operator_journey.py`, `backend/tests/fixtures/native_swe_repo/` | Branch-local real bounded path: exact service/session/approval binding, source/patch/path/command/resource checks, durable job fences/artifacts, actual child-process cleanup, failure/readback evidence, and replay dedupe. Arbitrary repository execution, host isolation, durable approval-route adoption, and external provider transport remain excluded. |
