...
- [x] active workflow triage now also exposes direct `use output`, `use failure`, `retry step`, `repair step`, `repair replay`, `open best`, `continue best`, `compare best`, `draft next step`, and keyboard-first latest-branch/failure/recovery follow-through so routine workflow recovery does not require an inspector hop for every next action
