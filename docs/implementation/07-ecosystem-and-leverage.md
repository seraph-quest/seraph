...
- [x] extension-backed memory providers now also expose operator-readable freshness/writeback quality controls, a provider-neutral adapter model with per-capability sync contracts, a canonical-memory/provenance contract, usefulness-ranked retrieval diagnostics, query-matched canonical project hints for provider-backed user modeling, and guarded additive writeback semantics that suppress project-scoped memories without an explicit project anchor instead of acting like opaque “connected or not” external memory seams
