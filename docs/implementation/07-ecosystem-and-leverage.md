...
- [x] cockpit workflow surfaces now promote step-focus debugging, direct step/output handoff actions, bundled family next-step planning drafts, denser ancestor/peer/failure-lineage follow-through, and active-triage workflow quick actions including failure-context reuse, direct best-continuation control, direct recovery actions, and keyboard-first best-continuation/comparison follow-through, so operators can act on the hottest failed or recoverable workflow step without reconstructing that context from raw history
