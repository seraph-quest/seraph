...
|---|---|---|---|---|
| Operator visibility and legibility | `[ ]` | audit trails, visible tool streaming, session traces, guardian cockpit shell, linked evidence panes, workflow-run views, operations inspector, and persisted cockpit layout presets | denser linked state, evidence, trace, artifact, and approval control surfaces plus deeper workflow history | 06, 03, 07 |
| Longitudinal memory and human modeling | `[ ]` | soul, vector memory, consolidation, goals, observer context, explicit guardian state, first observer salience/confidence scoring, a first calibrated aligned-work/high-salience pass, and a first explicit world-model layer for focus, commitments, pressure, and receptivity | deeper adaptive world-model quality and richer multi-signal learning over time | 05 |
...
| Workflow composition and delegation | `[ ]` | specialists, MCP, skills, delegation foundations, first-class reusable workflows, first cockpit workflow-run/operator views, and direct artifact-to-workflow draft handoff from the cockpit | deeper operator-facing workflow history and easier extension ergonomics | 07, 02 |
