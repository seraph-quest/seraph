...
- [ ] still missing: richer direct step-level workflow control, deeper workflow history, broader marketplace depth, easier extension ergonomics, and broader authenticated connector execution beyond the first connector-backed read bridge plus bounded work-item actions/report-publication path and the shipped step records, branch-family supervision, workspace-level workflow orchestration, artifact source/family follow-through, checkpoint branch controls, first cockpit timeline, operator terminal, plus the newer package-health/governance surfaces
