...
const MODES: { value: ApprovalMode; label: string; desc: string }[] = [
  { value: "high_risk", label: "High Risk", desc: "Approve shell, secret, and MCP actions" },
  { value: "off", label: "Off", desc: "Run risky tools without confirmation" },
];

export function ApprovalModeToggle() {
  const [mode, setMode] = useState<ApprovalMode>("high_risk");
  ...
  const handleSelect = async (m: ApprovalMode) => {
