...
└──────────────────────────────────────────────────────────────────────────────┘
                  ...
                  ▼
┌────────────────┐ ┌────────────────┐ ┌────────────────┐ ┌────────────────┐
│ Declarative    │ │ MCP            │ │ Managed        │ │ Trusted code   │
│ extensions     │ │ connectors     │ │ connectors     │ │ plugins        │
├────────────────┤ ├────────────────┤ ├────────────────┤ ├────────────────┤
│ skills         │ │ MCP servers    │ │ Slack          │ │ native tools   │
│ workflows      │ │ resources      │ │ Jira           │ │ deep runtime   │
│ runbooks       │ │ prompts/tools  │ │ GitHub         │ │ code hooks     │
│ starter packs  │ │ stdio/http     │ │ Google         │ │ later only     │
│ presets        │ │ long-tail use  │ │ observer src   │ │ explicit trust │
│ routines       │ │ repair flows   │ │ channel adapt. │ │ heavily gated  │
└────────────────┘ └────────────────┘ └────────────────┘ └────────────────┘
```
...
```text
┌────────────────────┬────────────────────┬────────────────────┬────────────────────┐
│ Dimension          │ MCP                │ Managed            │ Trusted code       │
├────────────────────┼────────────────────┼────────────────────┼────────────────────┤
│ Goal               │ ecosystem breadth  │ curated UX + trust │ deep host hooks    │
│ Install UX         │ often rough        │ polished           │ explicit / heavy   │
│ Auth owner         │ shared w/ server   │ Seraph-owned       │ plugin/core        │
│ Update path        │ per server         │ first-party        │ plugin lifecycle   │
│ Context tax        │ can be high        │ controlled         │ controlled         │
│ Safety posture     │ standard only      │ tighter fit        │ highest risk       │
│ Best for           │ long-tail tools    │ top apps           │ rare deep hooks    │
│ Default?           │ yes, for breadth   │ yes, curated       │ no                 │
└────────────────────┴────────────────────┴────────────────────┴────────────────────┘
```
