...
This prompt is intentionally feature-only until parity is reached. It supersedes the earlier proof-first parity goal wording. The goal is not to produce more proof surfaces as the main work, and it is not to pair each feature with a minimal proof companion task. The goal is to ship the user-facing parity capabilities Seraph still lacks and make those capabilities coherent with the guardian-workspace vision. Receipts, expanded tests, claim-ledger updates, and docs happen only after the full feature-parity implementation is complete, as a separate honesty and claim-readiness pass.

```text
Goal: complete the post-EF feature-parity implementation train needed to make production-grade feature parity and targeted exceedance possible for Seraph against Hermes, OpenClaw, and IronClaw, while preserving Seraph's guardian-workspace vision. Work feature-only first: implement the operator-facing capabilities, workflows, controls, runtime behavior, recovery paths, channels, memory behavior, security behavior, marketplace flows, and browser/computer-use behavior before adding proof wrappers, expanded docs, or claim-readiness layers. Do not stop at planning. Complete the feature implementation train first, then run the separate honesty and claim-readiness pass.
...
- If an issue, branch, or agent proposes another receipt, benchmark, proof endpoint, claim-lift gate, or evidence-only batch before the feature train is complete, stop and rescope it into a concrete user-facing capability batch or defer it to the final claim-readiness phase.
...
8. Claim-readiness phase only after the feature train has shipped.
   - This phase must not start until the feature batches above have materially shipped user-facing capability, not merely issue updates, operator proof endpoints, benchmark suites, or claim gates.
   - Refresh current Hermes/OpenClaw/IronClaw sources with URLs and access dates.
   - Run one minimal claim-honesty pass: current-source check, focused smoke/regression checks for changed surfaces, board/PR reconciliation, false-completion scan, and exact blocked-claim wording.
   - Do not add new proof endpoints or harnesses unless they are strictly required to keep claims honest after the feature train ships.
...
- The feature phase is complete when the post-EF feature-only train ships its intended user-facing parity capabilities and is merged.
- The overall goal is complete only after the separate final claim-readiness phase reconciles board state, focused changed-surface checks, docs, current competitor sources, false-completion scans, and exact claim-ledger wording.
```
