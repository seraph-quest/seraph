...
- shipped foundations: approvals, audit logging, secret redaction, secret-reference containment, sandboxed execution, background-process partitioning, disposable worker roots, credential-egress allowlists, managed connector boundary metadata, replay drift blocking on privileged surfaces, and Batch BW secure-host hardening receipts
...
- claim boundary: this milestone proves deterministic secure-host choke points, Batch BW hardening receipts, claim-bounded host-isolation strategy, and operator-visible receipts; it does not claim full host/container isolation, secure-by-default production posture, or complete browser/provider credential isolation
...
- shipped in the durable workflow-kernel v1 slice on this branch: workflow execution writes persisted run, step, safe checkpoint, retry or repair state, and durable audit receipt metadata while redacting unsafe secret or authenticated checkpoint payloads; normal delegated workflow execution now emits delegated artifact-review lifecycle rows with owner, reviewer, lineage, approval handoff, and trust-partition metadata; checkpoint resume tries durable state before audit projection; `/api/operator/durable-workflow-engine` and `durable_workflow_engine_v1` make deterministic proof fixtures and recent persisted state snapshots operator-visible
- missing strategic gaps: live heartbeat and reactive trigger executors remain future work beyond receipt-only trigger proof, workflows are still not a production distributed workflow engine, and deeper step-level execution control plus richer multi-session repair remain open
