...
- [x] workflow execution now also writes a minimal durable workflow state kernel: `workflow_run_states` and `workflow_step_states` persist run identity, lineage, step state, safe checkpoint context, and retry or repair state while redacting unsafe secret or authenticated checkpoint payloads; the durable repository and deterministic fixture proof include heartbeat-receipt and delegated artifact-review lifecycle shapes, checkpoint resume now tries durable state before audit projection, and `/api/operator/durable-workflow-engine` plus `durable_workflow_engine_v1` expose the bounded proof without claiming a full distributed workflow engine
