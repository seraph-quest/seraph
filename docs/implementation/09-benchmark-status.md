...
| Dense interface efficiency | `[ ]` | guardian cockpit default with linked state, workflow runs, interventions, audit, trace, fixed composer, persisted `default` / `focus` / `review` layouts, keyboard layout switching, per-layout save/reset, a first actionable desktop-shell rail, a searchable capability palette, a denser operator terminal, and legacy village fallback | stronger workflow-operating density, richer artifact/workflow history, and more flexible operator workspace control | 06, 07 |
