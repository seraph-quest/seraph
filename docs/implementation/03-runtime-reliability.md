...
- [ ] **Partial, branch-local #742; intended until the epic integration PR lands:** the workspace registry accepts the configured production bind, classifies canonical/derived/cache/secret state, emits deterministic redacted ready/degraded/blocked inventory receipts, and blocks unknown entries, symlinks, invalid roots, and missing required secret/recovery material. Managed `./manage.sh -e prod backup` and `restore --archive ... --confirm` now resolve only `BACKEND_DATA_PATH_PROD` (the backend mount is exactly `/app/data`), fence duplicate maintenance owners, create checksummed redacted archives, and stage/verify/promote restores with journaled rollback. Optional integration credentials such as `google_calendar_token.json` remain degraded when absent and are not recreated from an archive. Migration/write fencing, retention/disk-pressure drills, derived-index rebuild, session invalidation, production restore drill, and live operator receipts remain partial #742 acceptance work.
