...
class _DispatchVisitor(ast.NodeVisitor):
    ...
    def _collect_bindings(self, tree: ast.AST) -> None:
        ...
        # Resolve only simple, syntactic names. A fixed point handles an alias
        # chain without pretending to understand dynamic attribute or container flow.
        changed = True
        while changed:
            changed = False
            for node in nodes:
...
_REVIEWED_EXCEPTIONS = (
    ...
    _review(
        ...
        "client.post",
        "capability_probe",
        "Manual capability canary sends a bounded fixed fixture through run_capability_probe, which owns remote-inference admission.",
        "Retain as a leaf transport; all generating probe callers must use run_capability_probe or another shared admission adapter.",
    ),
    _review(
        "api/model_fabric_settings.py",
        953,
        ...
        "client.stream",
        "capability_probe",
        "Manual streaming capability canary observes one bounded fixed response through run_capability_probe, which owns remote-inference admission.",
        "Retain as a leaf transport; all generating probe callers must use run_capability_probe or another shared admission adapter.",
    ),
    _review(
        "api/model_fabric_settings.py",
        ...
        "client.post",
        "capability_probe",
        "Manual non-streaming chat capability canary sends a bounded fixed fixture through run_capability_probe, which owns remote-inference admission.",
        "Retain as a leaf transport; all generating probe callers must use run_capability_probe or another shared admission adapter.",
    ),
    _review(
