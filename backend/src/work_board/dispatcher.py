    ...
    async def _execute_direct_adapter(
        ...
        *,
        ...
        if capability_id == "guardian.research-watch.v1":
            ...
            try:
                occurrence = uuid.UUID(attempt.attempt_id).hex
            except (ValueError, AttributeError):
                occurrence = hashlib.sha256(attempt.attempt_id.encode()).hexdigest()[:32]
    ...
    async def _lookup_direct_job_id(
        ...
        try:
            existing = await lookup(
                ...
                run_fingerprint=self._direct_input_digest(task, attempt, inputs),
            )
