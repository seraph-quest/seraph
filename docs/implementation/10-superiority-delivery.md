...
- [x] The roadmap now refreshes to `capability-pack-autoinstall-and-bootstrap-v3` as the next active item because operator leverage is now high enough that the biggest remaining capability gain comes from a fuller install-doctor/bootstrap surface, with safety remaining immediately behind it.
- [x] The refreshed queue keeps fuller capability bootstrap/install depth, execution hardening, extension authoring, workflow branching/resume, cockpit density, provider explainability, native reach, deeper world-model fusion, stronger guardian learning, and broader behavioral proof in the top 10 because those remain the highest-value gaps after this batch lands.
