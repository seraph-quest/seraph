...
- [x] Seraph now also has a named guardian learning-quality receipt lane: `guardian_world_model_learning_quality_v2` and `/api/operator/guardian-learning-quality` expose deterministic fixture receipts for multi-signal learning arbitration, stale/conflicting evidence suppression, salience/confidence calibration, and false-positive/false-negative accounting without claiming live human-outcome validation or a completed adaptive world-model learning engine.
...
- [x] Governed-extension marketplace hardening foundations now add concrete local review receipts, compatibility and downgrade transition semantics, supply-chain/source-policy verdicts, provider trust-downgrade blockers, scaffolded local governance metadata, doctor guidance, and expanded M9 deterministic receipts while keeping production marketplace security as future proof work.
...
- [x] guardian learning quality now also has a dedicated operator-visible receipt surface: the `guardian_world_model_learning_quality_v2` suite catalogs multi-signal source mixes, stale-provider suppression, high-salience/partial-confidence clarification, false-positive and false-negative labels, and fixture-only claim boundaries rather than hiding policy-change justification inside M8 or user-model receipts.
...
- [ ] stronger learning and feedback loops beyond the current evidence-weighted delivery/channel/timing/blocked-state/suppression layer, the new multi-day and scheduled-outcome watchpoint pass, the first live-versus-durable procedural arbitration pass, the inspectable learning-diagnostics layer, and the named guardian learning-quality replay receipts
...
- [ ] The biggest gaps against the reference systems are now production-grade durable orchestration beyond the v1 state kernel, deeper execution hardening, denser step-level workflow/operator debugging beyond the new triage and evidence-control layer, live long-horizon intervention learning beyond deterministic guardian learning-quality and M8 receipts, and production-grade hardening beyond the broadened reach and marketplace foundations.
- [ ] The next major step is to land the stacked strategy PR train and then choose the next milestone from production-grade orchestration, execution isolation, live learning, or connector/marketplace production hardening.
