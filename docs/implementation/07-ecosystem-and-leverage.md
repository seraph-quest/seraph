...
- [x] M9 now also ships governed-extension marketplace hardening foundations: review receipts with digest/key/review/permission/compatibility/supply-chain verdicts, update transition blockers for verified/provider-backed trust downgrades and version downgrades, scaffolded local governance placeholders, doctor guidance for source/supply-chain policy failures, and expanded deterministic M9 receipts without claiming a production-secure marketplace
...
- [ ] richer extension health/test surfaces, deeper marketplace curation, production marketplace security, and production-grade connector hardening beyond the current lifecycle API, governance queue, review receipts, compatibility transition blockers, supply-chain/source-policy doctor guidance, diagnostics summaries, operator health triage, marketplace-flow composition, M9 governed-ecosystem proof, and deterministic eval coverage
...
- [ ] clearer extension ergonomics for third-party and user-authored capabilities beyond the current local governance scaffold placeholders, operator surface, repair actions, live logs, runbooks, preflight surfaces, diagnostics, extension studio, and skill-registry loop
