...
- [x] benchmark proof on `develop` now also has an operator-visible suite/report layer, a dedicated guardian-memory benchmark with workflow, approval, audit, artifact, and repo or PR continuity retrieval plus contradiction or forgetting diagnostics, a dedicated M6 memory-superiority benchmark for long-horizon recall, contradiction handling, stale-memory override, source trust/privacy boundaries, provider-quality diagnostics, and receipted behavior change, a dedicated workflow-endurance benchmark with anticipatory-repair and backup-branch failure taxonomy, a dedicated trust-boundary benchmark with operator safety receipts and failure taxonomy, a dedicated computer-use benchmark with replayable browser/desktop execution receipts and failure taxonomy, and a dedicated governed-improvement benchmark with anti-misevolution blocking, canary-only rollout posture, rollback-ready receipt policy, and recent saved proposal receipts instead of leaving deterministic proof distributed across raw eval-scenario names
