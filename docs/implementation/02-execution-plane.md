...
- [x] Native software-engineering service runs persist the canonical service execution owner and stable service session in the durable job. The separate approval row is owned by the authenticated interactive operator session/principal and binds to that service authority; the native consumer supplies both bindings, while generic approval selection and consumption remain fail-closed for missing or mismatched identity fields.
