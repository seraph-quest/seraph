...
- [x] The cockpit now exposes denser workflow-operating control through step-focus summaries, direct failed-step context handoff, direct output reuse and output-comparison drafts from workflow family rows, explicit branch-origin and failure-lineage debugging, family-history comparison, and best-continuation controls instead of leaving those actions buried in the inspector.
