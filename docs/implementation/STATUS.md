...
- [x] Execution hardening now also covers scoped extension mutation approvals, connector-backed authenticated mutation planning, explicit trust-boundary payloads across workflow/operator/activity surfaces, operator-visible approval scope metadata, and heavier CI stabilization for long-tail backend/frontend suites instead of only replay blocking plus generic pending-approval rows.
